...
MovieAPIKey='ENTER YOUR MOVIE API KEY'
GoogleAPIKey = 'ENTER YOOUR GOOGLE API KEY'
...
def search_locations():
    ...
    if q:
        locations = db.session.query(Location).filter(Location.city.ilike(f'{q}%')).distinct().limit(20)
    ...
    return render_template('locations.html', posts=post_list)

@app.route('/api/get_locations')
def api_get_locations():
    """API to get close locations:: 
    """
